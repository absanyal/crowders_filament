...
for run_i_m1 in range(0, runmax):
    ...
    t, e1x, e1y, e1z, e2x, e2y, e2z = np.loadtxt('e2e_pos/e2e_pos.{}.txt'.format(run_i), unpack=True)
    ...
    with open('ang_deviation.{}.txt'.format(run_i), 'w') as f:
    ...
    with open('e2e_dist.{}.txt'.format(run_i), 'w') as f:
        f.write('#t e2e_dist\n')
        for i in range(len(t)):
            hv = np.array([e1x[i] - e2x[i], e1y[i] - e2y[i], e1z[i] - e2z[i]])
            f.write('{}\t{:.4f}\n'.format(t[i], np.linalg.norm(hv)))

    t, e2e_dist_list = np.loadtxt('e2e_dist.{}.txt'.format(run_i), unpack=True)
    ...
    plt.plot(t, e2e_mean_line, 'r--', linewidth=2, label='Mean length: {:.2f}'.format(np.mean(e2e_dist_list)), alpha=0.7)
    plt.xlabel(r'$t/\tau$', fontsize=18)
    plt.ylabel("End to end distance", fontsize=18)
    plt.xlim(0, max(t))
    plt.legend(loc='best', markerscale=2)

    plt.savefig('e2e_pos.{}.pdf'.format(run_i))
